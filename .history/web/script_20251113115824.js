...
function findOrCreateClient(name, phone, email) {
	name = (name || '').trim();
	let client = state.clients.find(c => c.name.toLowerCase() === name.toLowerCase() && c.email === email);
	if (!client) {
...
function renderOrders() {
	...
	const rows = state.orders.filter(o => {
		...
		const hay = (client.name + ' ' + o.itemType + ' ' + o.notes + ' ' + o.measurements).toLowerCase();
		return hay.includes(search);
	});

	for (const o of rows) {
		const client = state.clients.find(c => c.id === o.clientId) || { name: '—' };
...
function renderClients() {
	...
	for (const c of state.clients) {
		...
		tr.innerHTML = `
			...
			<td>${lastOrder ? formatDate(lastOrder.dueDate) + ' — ' + escapeHtml(lastOrder.itemType) : ''}</td>
		`;
		tbody.appendChild(tr);
	}
}
