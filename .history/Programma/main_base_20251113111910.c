...
typedef struct{
    idRitaglio (char[50]) = Codice identificativo del ritaglio, generato 
automaticamente per tracciarlo nel sistema. 
●  id_rotolo (char[50]) = Riferimento al rotolo da cui il ritaglio è stato ottenuto. 
●  lunghezza (float) = Misura del ritaglio (in metri), utile per eventuali riutilizzi 
o vendite di scarti. 
●  dataCreazione(struct data) = Contiene giorno, mese e anno in cui il ritaglio 
è stato registrato.
} t;
typedef struct{
    char nome[50];
