...
typedef struct{
    id  (char[50]) = Codice identificativo univoco del prelievo 
●  id_rotolo (char[50]) = Collegamento al rotolo da cui è stato prelevato il 
tessuto. 
●  metraggio_prelevato (float) = Lunghezza del tessuto prelevata (metri) 
●  data (struct data) = Contiene giorno, mese e anno del prelievo. 
●  operatore (char[50]) = Nome dell’operatore che ha eseguito il prelievo
} t_Prelievo;
typedef struct{
    char nome[50];
    char partita_iva[20];
