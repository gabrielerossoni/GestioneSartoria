...
typedef struct{
    data_controllo (struct data) = Indica la data dell’ultimo inventario o 
controllo del magazzino. 
●  rotoli (struct rotolo[200]) = Array contenente fino a 200 rotoli presenti nel 
magazzino, ognuno con tutte le informazioni del tessuto. 
●  valoreTotale (float) = Valore monetario totale dei tessuti in magazzino. 
●  metraggioTotale (float) = Somma dei metri disponibili di tutti i rotoli. 
●  numeroRotoli (int) = Numero effettivo di rotoli attualmente presenti. 
●  priorita_utilizzo (int) = Indica la priorità con cui utilizzare i tessuti: 
1.  prima (da utilizzare subito) 
2.  normale 
3.  scorta (da usare solo quando necessario)
} t_Inventario;
} t
typedef struct{
    int giorno;
