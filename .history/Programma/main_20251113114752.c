...
int modificaRotolo(t_Rotolo[])
{
    int i;
    char id[MAX_CARATTERI];
    printf("INSERISCI ID ROTOLO DA MODIFICARE: ");
    scanf("%s", id);
    // Logica per modificare il rotolo con l'ID specificato
    for(i=0; i<MAX_ROTOLI; i++) {
        // Supponiamo che rotoli[i].id sia accessibile qui
        // if(strcmp(rotoli[i].id, id) == 0) {
        //     // Modifica i campi del rotolo
        // }
    }
    return 0;
}